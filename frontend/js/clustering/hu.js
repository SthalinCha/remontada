/**
 * clustering/hu.js
 * Responsabilidad: Lógica específica de Momentos de Hu
 * - Análisis de imágenes con Hu
...
export function initHu() {
  const addImagesBtn = document.getElementById("add-images-btn-hu");
  const updateCapacitiesBtn = document.getElementById("update-capacities-btn-hu");
  const statusBtn = document.getElementById("status-btn-hu");
  ...
  if (updateCapacitiesBtn) {
    updateCapacitiesBtn.addEventListener("click", () => {
      const capacitiesInput = document.getElementById("hu-capacities-input");
...
export async function analyzeHu(files, capacities = null) {
  try {
    setStatus("Analizando con Momentos de Hu...");
    clearResults();
    clearResultsDisplay();
    ...
    if (data.results && data.results.length > 0) {
      ...
      setStatus(`✅ ${data.results.length} imágenes analizadas con Hu`);
    }
  } catch (error) {
...
async function addImages(files) {
  ...
  try {
    setStatus(`Agregando ${files.length} imágenes al clustering Hu...`);
...
async function showClusterStatus() {
  try {
    ...
    const statusDisplay = document.getElementById("cluster-status-display-hu");
    const statusContent = document.getElementById("cluster-status-content-hu");
    
    if (statusDisplay && statusContent) {
      statusContent.innerHTML = `
        <strong>Estado del Clustering (Hu):</strong><br>
