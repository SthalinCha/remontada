/**
 * clustering/cnn.js
 * Responsabilidad: Lógica específica de CNN/ResNet50
...
export function initCnn() {
  const addImagesBtn = document.getElementById("add-images-btn-cnn");
  const updateCapacitiesBtn = document.getElementById("update-capacities-btn-cnn");
  const statusBtn = document.getElementById("status-btn-cnn");
  ...
  if (updateCapacitiesBtn) {
    updateCapacitiesBtn.addEventListener("click", () => {
      const capacitiesInput = document.getElementById("cnn-capacities-input");
...
export async function analyzeCnn(files, capacities = null) {
  try {
    setStatus("Analizando con CNN/ResNet50...");
    clearResults();
    clearResultsDisplay();
    ...
    if (data.results && data.results.length > 0) {
      ...
      setStatus(`✅ ${data.results.length} imágenes analizadas con CNN`);
    }
  } catch (error) {
...
async function addImages(files) {
  ...
  try {
    setStatus(`Agregando ${files.length} imágenes al clustering CNN...`);
...
async function showClusterStatus() {
  try {
    ...
    const statusDisplay = document.getElementById("cluster-status-display-cnn");
    const statusContent = document.getElementById("cluster-status-content-cnn");
    
    if (statusDisplay && statusContent) {
      statusContent.innerHTML = `
        <strong>Estado del Clustering (CNN):</strong><br>
